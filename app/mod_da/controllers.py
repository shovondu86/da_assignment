...
def getAdvIndicator():
    ...
    avgcatV=db.session.query(0,func.round(func.avg(DataCases.number_morbidity),2)).filter(DataCases.species=='cat').filter(DataCases.location.ilike('%Village%')).all()
    
    #Average number of sick cats report to advanced indicator dictionary
    #Convert to dictionary
    avgcatVD=dict(avgcatV)
    avgcatVD["Average number of sick cats reported in reports from villages up to two decimal points"] = avgcatVD.pop(0)
